...
function handleGetdetail() {
    ...
    handleRequest(skuId, "TB", text => {
        ...
        try {
            ...
            //鏂规硶1
            try {
                body.data.propsCut = `浠锋牸璇︽儏 ${body.data.propsCut}`;
                body.data.props.groupProps[0]["鍩烘湰淇℃伅"].unshift({ "浠锋牸璇︽儏": text });
            } catch (e) {
        ...
        if (body.data.apiStack) {
            ...
            try {
                ...
                //Body
                let consumerProtectionTBItem = createConsumerProtectionTBItem("浠锋牸璇︽儏", text);
                let consumerProtectionTBItemList = createConsumerProtectionTBItem("浠锋牸璇︽儏", [text]);
            ...
            try {
                ...
                // try {
                //     //鏂规硶1
                //     data.resource.entrances.vip95CardBanner = createTBCard("浠锋牸璇︽儏", text);
                // } catch (e) {
                ...
                try {
                    //鏂规硶2
                    const regex_tips = /Tips(.*)浠锋牸鍖洪棿/gm;

                    let tips = text.replaceAll("\n"," ").match(regex_tips)[0];

                    tips = tips.replaceAll("浠锋牸鍖洪棿","");

                    data.componentsVO.bannerBeltVO = createTBCard(`${tips}锛岃鎯呰<淇濋殰>鎴 <鍙傛暟>`, "");
                } catch (e) {
...
function request_history_price(id, type, callback) {
    ...
    getBijiagoCookie(cookie => {
        ...
        $.get(option, (err, rsp, data) => {
            ...
            //     result.success = false;
            //     result.msg = "鑾峰彇浠锋牸淇℃伅澶辫触锛孋ookie澶辨晥锛岃鍒锋柊椤甸潰";
            //     set_tag("cookie", "");
            // }
